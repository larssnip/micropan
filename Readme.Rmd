...
readFasta(file.path("faa", str_c(gnm.tbl$GenBank_ID[1], "_", gnm.tbl$GID.tag[1], ".faa"))) %>% 
  head()
```
It is the files in this folder you will need for the rest of the analysis, and everything inside the `tmp/` folder may now be deleted.
...
Storing such results in files in a separate subfolder is a good idea. If you choose to add more genomes later, these results are already there, and will not be re-computed. Never put any other files into this folder! If you want to re-compute something, delete the corresponding files in the `out.folder` first.
...
The `complete`linkage means we have exact control of the 'diameter' of our clusters or gene families. The other linkages are less transparent in this sense.
